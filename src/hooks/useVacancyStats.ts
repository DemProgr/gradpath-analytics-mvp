        ...
        if (rpcError) {
           ...
           const aggregatedStats: VacancyStats[] = Array.from(categoryMap.entries())
             .map(([category, data]) => {
               ...
               let avgSalaryMax = data.validSalaryCount > 0 
                ...
                console.log('[useVacancyStats]', category, '→ raw:', avgSalary, '| floor:', floor, 'cap:', cap, '| final:', avgSalary);
                
                // Highlight engineering/economics
                if (category.includes('Инжен') || category.includes('Engineer') || category.includes('Эконом')) {
                  console.log('[useVacancyStats] >>>', category, '| raw:', avgSalary, '→ final:', avgSalary);
                }
          ...
          console.log(`[useVacancyStats] Client-side: Loaded ${allVacancies.length} vacancies, ${aggregatedStats.length} categories`);
            ...
            const formattedStats: VacancyStats[] = (rpcData || []).map((row: any) => {
              ...
              // Debug all
              console.log('[useVacancyStats RPC]', category, '→ raw:', avgSalary, '| floor:', floor, 'cap:', cap, '| final:', avgSalary);
